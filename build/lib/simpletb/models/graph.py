...
class SpatialGraph:
    ...
    def __init__(self, cellpath, pointpath ):
        
        self.primitive_vectors = np.loadtxt(cellpath).T # column vectors
        self.cart_points  = np.loadtxt(pointpath).T # column vectors
        self.frac_points = np.linalg.inv(self.primitive_vectors.T)@(self.cart_points) # column vectors
        self.metric = ((self.primitive_vectors).T)@(self.primitive_vectors)
